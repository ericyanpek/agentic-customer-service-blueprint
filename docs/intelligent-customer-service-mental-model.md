...
动作风险等级回答“这个动作能不能自动执行”，任务能力等级回答“Agent 要接触到什么才可能解决这个问题”。两者是不同的轴：同一个风险等级下的任务，能力要求可以差一个数量级。为避免与风险轴的 `L` 混用，能力轴使用 `C` 前缀。

| 等级 | Agent 需要接触什么 | 新增的硬门槛 | 典型问题 |
|---|---|---|---|
| C0 | 只需正式知识，不接触任何玩家数据 | KB 质量、引用与生效期 | 活动几点结束、道具合成规则 |
| C1a | 读当前玩家自有数据，单次查询即可判断 | 可信身份绑定 | 查充值记录、查订单状态 |
| C1b | 读当前玩家自有数据，需要多步调查 | 调查路径收敛 | 比对订单、发货流水与背包，判断卡在哪一步 |
| C2 | 可逆写操作 | 幂等键 | 建单、改单、重试可逆任务 |
| C3 | 不可逆写操作 | Ledger、状态条件、审批、补偿 | 补发、退款、账号解绑 |

##### 阶梯成本不是线性的

| 跨越 | 代价 | 说明 |
|---|---|---|
| C0 → C1a | 一次性门槛 | 风险性质从“说错”变为“越权读”。跨玩家数据泄露比动作出错更难挽回，因此权限过滤必须 fail closed：过滤条件返回空集时不得降级重试 |
| C1a → C1b | 中等 | 不增加新的安全面，增加的是编排质量要求。这一跳才是 Agent Loop 相对 RAG 的分界线 |
| C1b → C2 | 低 | 写工具自身一张幂等表即可，不必先建独立的 Side-effect 平面 |
| C2 → C3 | 最贵，约十倍 | Ledger、Workflow、补偿、错误分类、Policy 强制执行和故障注入验证几乎全部集中在这一跳 |
| C3 → 人工决定 | 反而下降 | Agent 交出决定权，不再需要授权与事务保证 |

因此“做到哪一层”的实质决策点只有一个：是否跨越 `C2 -> C3`。

##### 转人工不是能力等级

转人工是每一层都存在的出口，不是 C3 之后的第四层：

- C0 也会转人工：取不到知识引用、客户明确要求、情绪持续升级。
- C1b 也会转人工：事实已查清，但结论超出自动化授权范围。

把转人工理解为“前面几层都失败之后的兜底”，会漏掉大部分真实触发条件。

转人工本身有三个层次，只有第三层能覆盖高风险场景：

```text
冷转     ：仅移交会话，人工从零开始
带上下文 ：摘要、已验证事实、引用和已尝试动作
带建议   ：Agent 已完成调查，给出建议动作，由人工决定
```

由此得到一条与风险轴的对应关系：

> L3 / L4 动作的正确形态不是“Agent 不参与”，而是 `C1b 调查 + 建议 + 人工决定`。

它在客户体验上已经是“智能客服处理了退款”，在工程上不包含任何写操作，也不需要跨越 `C2 -> C3`。这同时说明人工坐席增强与客户自助不是两条产品线，而是同一套 KB、Skills、工具和 Trace 的两个消费面。

##### 能力等级决定评测方式

| 等级 | 主要评测对象 |
|---|---|
| C0 | 引用正确率、Faithfulness、知识缺口率 |
| C1a | 工具选择准确率、参数正确性、跨玩家访问必须为零 |
| C1b | 调查路径是否收敛：是否漏查关键步骤、是否重复绕圈、多少轮内得出结论 |
| C2 | 重复执行率、确认步骤生效率 |
| C3 | 错误动作率、Side-effect 恢复率、补偿成功率 |

`C1a` 与 `C1b` 的风险等级相同但评测方式不同，这是把它们分开列出的原因。
